        ...
        if (_kbhit()) {
            ...
            if (c == 0 || c == 0xE0) { // Special key
                ...
                if (next == 72) { // UP
                    ...
                    if (pos < len) { pos++; printf("\033[C"); } // If ANSI enabled, else need cursor API
                }
                continue;
            }
        } else {
        ...
        if (c == 3) { // CTRL-C
            ...
            // Check for explicit CTRL-H on POSIX (ASCII 8) or if user meant Help
             ...
             if (pos > 0) {
                 ...
                 // Correct way: use \r then print prompt + buffer + space + backspace makes sense for end of line
                 // But complex for middle edits. 
                 // Simple hack: The above redraws line and puts cursor at end.
                 // Correct cursor position:
                 int i;
                 for(i=0; i< (len-pos); i++) printf("\b");
             }
             continue;
             #else 
